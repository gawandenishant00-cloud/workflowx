create table if not exists profiles (id uuid references auth.users primary key, full_name text not null, role text not null check (role in ('admin','manager','employee')), department text, created_at timestamptz default now());
...
create table if not exists purchase_requests (id uuid default gen_random_uuid() primary key, requester_id uuid references profiles(id) not null, vendor_id uuid references vendors(id), item_description text not null, amount numeric not null check (amount > 0), status text default 'pending' check (status in ('pending','approved','rejected')), created_at timestamptz default now());
